 ...
 *  ZMQ 協定：
 *    發送 (C → Python):
 *      {"node_id": 3586, "ues": [{"rnti": X, "bsr": Y, "wb_cqi": Z}, ...]}
 *    接收 (Python → C):
...
#define ZMQ_ENDPOINT      "ipc:///tmp/zmq_node2_inference.ipc"
...
static bool zmq_socket_init(void)
{
    ...
    if (g_zmq_sock == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] zmq_socket() 失敗: %s\n" CLR_RESET,
    ...
    if (zmq_connect(g_zmq_sock, ZMQ_ENDPOINT) != 0) {
        fprintf(stderr, CLR_RED "[Node2 xApp] zmq_connect(%s) 失敗: %s\n" CLR_RESET,
...
static void sm_cb_mac(sm_ag_if_rd_t const *rd)
{
    ...
    if (root == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] cJSON_CreateObject 失敗，Fallback\n" CLR_RESET);
    ...
    if (payload == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] cJSON 序列化失敗，Fallback\n" CLR_RESET);
        apply_fallback(num_ues, stats);
        return;
    }
    ...
    if (send_rc < 0) {
        ...
        fprintf(stderr, CLR_YEL "[Node2 xApp] ZMQ send 失敗 (%s)，Fallback\n" CLR_RESET,
                zmq_strerror(zmq_errno()));
        apply_fallback(num_ues, stats);
        return;
    }
    ...
    if (resp == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] 回傳 JSON 解析失敗，Fallback\n" CLR_RESET);
        apply_fallback(num_ues, stats);
        return;
    }
    ...
    if (!cJSON_IsArray(allocs_arr) || cJSON_GetArraySize(allocs_arr) == 0) {
        fprintf(stderr, CLR_RED "[Node2 xApp] 回傳格式無效 (缺少 'allocations' 陣列)，Fallback\n" CLR_RESET);
        cJSON_Delete(resp);
        apply_fallback(num_ues, stats);
        return;
    }
    ...
    if (req.msg.slices == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] calloc 失敗，Fallback\n" CLR_RESET);
        cJSON_Delete(resp);
        apply_fallback(num_ues, stats);
        return;
    }
    ...
    for (int i = 0; i < alloc_n; i++) {
        ...
        printf("[Node2 xApp] AI 決策 UE[0x%04x]: prb_abs=%.0f → ratio=%.3f\n",
...
int main(int argc, char *argv[])
{
    printf(CLR_CYAN
           "====================================================\n"
           "  Node 2 Local xApp — Near-RT 閉環 PRB 控制器\n"
           "  目標節點 ID : %u\n"
           "  ZMQ 端點    : %s\n"
           "  回報週期    : 10 ms\n"
           "====================================================\n"
    ...
    if (g_zmq_ctx == NULL) {
        fprintf(stderr, CLR_RED "[Node2 xApp] zmq_ctx_new() 失敗，程式退出\n" CLR_RESET);
        return EXIT_FAILURE;
    }

    if (!zmq_socket_init()) {
        fprintf(stderr, CLR_RED "[Node2 xApp] ZMQ socket 初始化失敗，程式退出\n" CLR_RESET);
        zmq_ctx_destroy(g_zmq_ctx);
        return EXIT_FAILURE;
    }
    printf(CLR_GREEN "[Node2 xApp] ZMQ REQ socket 已連線至 %s\n" CLR_RESET, ZMQ_ENDPOINT);
    ...
    while (target_idx == -1) {
        ...
        if (target_idx == -1) {
            printf(CLR_YEL "[Node2 xApp] 等待 Node 2 (ID=%u) 連線... "
                   "目前已連接節點數: %d，2 秒後重試\n" CLR_RESET,
    ...
    printf(CLR_GREEN "[Node2 xApp] 成功鎖定 Node 2 (nb_id=%u)\n" CLR_RESET,
    ...
    if (!sub_ans.success) {
        fprintf(stderr, CLR_RED "[Node2 xApp] 訂閱 MAC SM 失敗，程式退出\n" CLR_RESET);
        free_e2_node_arr_xapp(&nodes);
        if (g_zmq_sock != NULL) zmq_close(g_zmq_sock);
        zmq_ctx_destroy(g_zmq_ctx);
        return EXIT_FAILURE;
    }
    printf(CLR_GREEN "[Node2 xApp] MAC SM 訂閱成功 (handle=%d)，"
    ...
    if (pthread_create(&wd_tid, NULL, watchdog_thread, NULL) == 0) {
        ...
        printf(CLR_GREEN "[Node2 xApp] Watchdog 執行緒已啟動 (超時門檻=%ds)\n"
               CLR_RESET, WATCHDOG_TIMEOUT_S);
    } else {
        fprintf(stderr, CLR_YEL "[Node2 xApp] Watchdog 執行緒啟動失敗，繼續運行\n" CLR_RESET);
    }
     ...
     * ─────────────────────────────────────────────────────────────────── */
    printf("[Node2 xApp] 收到停止訊號，開始清理資源...\n");
    ...
    printf(CLR_GREEN "[Node2 xApp] 程式正常退出\n" CLR_RESET);
    return EXIT_SUCCESS;
}
